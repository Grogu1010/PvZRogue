window.__patchPvZRogueBossMythic = function patchPvZRogueBossMythic(source) {
  ...
  req(
    ...
    '  pineapplepuncher: { name: "Pineapple Puncher", icon: "🍍", cost: 400, hp: 1625, kind: "pineapple", damage: 6000, punchCd: 7, radius: 1 },\n  firepeashooter: { name: "Fire Peashooter", icon: "🔥", cost: 175, hp: 310, kind: "shooter", damage: 38, fireRate: 1.28, hitsAir: true, firePlant: true },\n};',
...
function zombieIntelStats(type, state) {
  ...
  return { hp:Math.round(st.hp), speed:Number(st.speed).toFixed(2), damage:Math.round(st.damage), unlock:def.unlock, tags:[st.flying?"Flying":null,def.heavy?"Heavy":null,def.boss?"BOSS":null,def.immovable?"No knockback":null,def.fireproof?"Fireproof":null,def.explosionProof?"Explosion-proof":null].filter(Boolean) };
}
...
`      if (next.poison) next.hp -= next.poison * dt;
  ...
  source = source.replace('        const viscoReflectsBeam = target.type === "viscoelastic"', '        if (target.type === "coolbrainz" && (pr.fire || pr.source === "firepeashooter")) { pr.pierce = 0; if (DEVICE_TIER !== "ultra") floaties.push({id:makeId("fireproof"),text:"FIREPROOF",x:target.x,y:target.row*CELL_H+8,life:.5}); continue; }\n        const viscoReflectsBeam = target.type === "viscoelastic"');
  ...
  req(
    ...
    'boss explosion immunity'
  );
  source = source.replace('z.hp -= stats.damage;\n            if (stats.mods.scorchSlow)', 'if (z.type !== "coolbrainz") z.hp -= stats.damage;\n            if (stats.mods.scorchSlow && z.type !== "coolbrainz")');
  source = source.replace('if (stats.mods.burnDps) z.poison = Math.max(z.poison, stats.mods.burnDps);', 'if (stats.mods.burnDps && z.type !== "coolbrainz") z.poison = Math.max(z.poison, stats.mods.burnDps);');
