...
window.__patchPvZRogueBossProjectileCompatAfter = function patchPvZRogueBossProjectileCompatAfter(source) {
  ...
  const swap = (from, to) => { if (source.includes(from)) source = source.replace(from, to); };
  ...
  if (source.includes(plantSpriteAnchor) && !source.includes('function FirePeashooterSprite(')) {
    const firePeaSprite = String.raw`
    ...
    <SpriteFrame small={small} action={attacking ? "attack" : "idle"}>
      ...
      <motion.g animate={attacking ? {y:[0,-2,0],rotate:[0,-2.5,0,2,0]} : {y:[0,-1,0],rotate:[0,1.2,0,-1.2,0]}} transition={{duration:attacking?.34:1.8,repeat:Infinity,ease:"easeInOut"}} style={{transformOrigin:"31px 39px"}}>
        ...
        <motion.g animate={attacking ? {x:[0,4,0],scaleX:[1,1.07,1]} : {x:[0,.7,0]}} transition={{duration:attacking?.19:1.3,repeat:Infinity}} style={{transformOrigin:"48px 28px"}}>
          <ellipse cx="48" cy="28" rx="13" ry="10" fill="url(#firePeaHead)" stroke="#8d2d14" strokeWidth="2.2"/><ellipse cx="56" cy="28" rx="5.2" ry="3.8" fill="#6a1e10"/><ellipse cx="56" cy="28" rx="2.5" ry="1.8" fill="#2f0e08"/>
